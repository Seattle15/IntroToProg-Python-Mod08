...
#                            from file
#                 08.26.2021 Modified code to more implicitly save as a list of product objects and
...
class Product():
    ...
    def __str__(self):
        ...
        product_info = product_name.strip() + ', ' + product_price.strip()
...
class FileProcessor:
        ...
        if os.path.exists(file_name):  # used to check if the text file exists and run script if True
            ...
            for line in file:
                if "," in line:  # added this condition as was getting an error when text file empty
                    ...
                    status = '\nThere is no data saved in the file.\n'
            file.close()
        else:
            status = '\nFile does not exist.\n' \
                     'File will be created once you save your list.\n'
    ...
    def add_product_to_list(product, price, lstOfProductObjects):
        ...
        lstOfProductObjects.append(new_product)   # save the instance object to list
    ...
    def remove_data_from_list(product, lstOfProductObjects):
        ...
        for row in lstOfProductObjects:       # in this function I did not treat rows as an object instance
            row_data = row.__str__()               # gets the product info saved as a string
            product_local, price = row_data.split(',')
...
while (True):
    ...
    elif strChoice == '2' and lstOfProductObjects != []:  # Remove an existing product if product list is not empty
    ...
    elif strChoice == '2' and lstOfProductObjects == []:  # Remove an existing product requested but list is empty
